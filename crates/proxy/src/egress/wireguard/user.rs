//! The **userspace** WireGuard backend: boringtun (protocol) + smoltcp (TCP/IP)
//! over a plain UDP socket — no TUN device, no privileges, fully in-process.
//! This is the portable fallback; on Linux with `CAP_NET_ADMIN` the kernel
//! backend (see [`super::kernel`]) is preferred for throughput.
//!
...
//! `SocketSet`, the packet-shuttling [`super::device::WgDevice`], and the UDP
//! socket. `connect()` asks that task (via `Ctrl::Open`) to open a virtual TCP
//! socket and hands back one end of a `tokio::io::duplex` pipe; a small
//! per-connection bridge task moves bytes between that pipe and the loop's mpsc
//! channels. The loop itself never does async per-connection I/O and never holds
//! a lock across an `.await` (freeze-safety).
//!
//! Both the traffic *and* the DNS lookup for a routed domain go through the
//! tunnel (see [`super::dns::TunnelResolver`]) — here the resolver's stream is a
//! virtual TCP socket opened through the tunnel loop.
...
/// The userspace WireGuard backend. Cheap handle; the tunnel runs in its own task.
pub(super) struct UserWg {
    id: String,
    ...
    /// Host → IP through the tunnel, cached by TTL (shared with the kernel backend).
...
enum Ctrl {
    ...
    Open {
        remote: SocketAddr,
        io: DuplexStream,
        reply: oneshot::Sender<std::result::Result<(), ConnectError>>,
    },
}
...
impl UserWg {
    /// Spawn the tunnel task for an already-parsed `.conf`. Must be called within
    /// a tokio runtime (it is — egresses are built at app init / API reload).
    pub(super) fn spawn(cfg: &EgressConfig, conf: WgConf, upstream: Arc<ZoneRouter>) -> Self {
        ...
        // NB: the supervisor holds ONLY the receiver. When this UserWg is dropped
    ...
    pub(super) async fn connect(
        ...
        host: &str,
        port: u16,
    ) -> std::result::Result<DuplexStream, ConnectError> {
    ...
    async fn open(&self, remote: SocketAddr) -> std::result::Result<DuplexStream, ConnectError> {
        ...
        self.ctrl
            ...
            .await
            .map_err(|_| {
                ConnectError::egress(FeriteError::Dns("wireguard tunnel is down".into()))
            })?;
        reply_rx.await.map_err(|_| {
            ConnectError::egress(FeriteError::Dns("wireguard tunnel did not respond".into()))
        })??;
        Ok(caller)
    }

    /// Resolve `host` through the tunnel — the resolver's DNS-over-TCP stream is
    /// a virtual socket opened via [`Self::open`]. Caching and upstream-fallback
    /// semantics live in [`TunnelResolver::resolve`].
    async fn resolve(&self, host: &str) -> Result<IpAddr> {
        self.resolver.resolve(host, |addr| self.open(addr)).await
    }
}
...
struct Conn {
    ...
    connect_reply: Option<oneshot::Sender<std::result::Result<(), ConnectError>>>,
        ...
        tokio::select! {
            ...
            msg = ctrl_rx.recv() => match msg {
                ...
                Some(Ctrl::Open { reply, .. }) => {
                    ...
                    let _ = reply.send(Err(ConnectError::egress(FeriteError::Dns(
...
async fn run_tunnel(
    ...
    // net.core.rmem_max/wmem_max.
    {
        ...
        if let Ok(rcv) = sock.recv_buffer_size() {
            let usable = super::super::usable_rcvbuf_bytes(rcv);
    ...
    loop {
        tokio::select! {
            ...
            ctrl = ctrl_rx.recv() => match ctrl {
                None => return RunOutcome::Shutdown, // UserWg dropped → tear down
...
fn open_conn(
    ...
    if let Err(e) = sock.connect(iface.context(), remote_ep, local_port) {
        let _ = reply.send(Err(ConnectError::destination(FeriteError::Dns(format!(
...
fn settle_connects(
    ...
    for (h, c) in conns.iter_mut() {
        ...
        if sock.may_send() {
            ...
            if let Some(tx) = c.connect_reply.take() {
                let _ = tx.send(Err(ConnectError::destination(FeriteError::Dns(
...
mod smoke {
    ...
    async fn idle_tunnel_stays_healthy_past_session_age() {
        ...
        let eg = UserWg::spawn(&egress(text), conf, upstream());
